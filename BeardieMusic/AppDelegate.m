...
//                        nil];
    ...
    NSDictionary *appDefaults = [NSDictionary dictionaryWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"BeardieMusicUserDefaults" ofType:@"plist"]];
    if (appDefaults)
        [[NSUserDefaults standardUserDefaults] registerDefaults:appDefaults];

    // Create serial queue for user actions
    workingQueue = dispatch_queue_create("com.beardiemusic.working.serial", DISPATCH_QUEUE_SERIAL);
    ...
    [registry setUserDefaults:BeardieMusicActiveControllers strategyCache:strategyCache];

    // setup native apps
    nativeAppRegistry = [NativeAppTabRegistry singleton];
    [nativeAppRegistry setUserDefaultsKey:BeardieMusicActiveNativeAppControllers];
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:BeardieMusicUpdateAtLaunch])
...
/////////////////////////////////////////////////////////////////////////
#pragma mark BeardieMusicHostAppProtocol methods
/////////////////////////////////////////////////////////////////////////
...
- (void)volumeUp{
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:BeardieMusicCustomVolumeControl]) {
...
- (void)checkAXAPIEnabled{
    ...
    if (_AXAPIEnabled){
        ...
        alert.informativeText = NSLocalizedString(@"Once you enable access in System Preferences, you must restart BeardieMusic.", @"Explanation that we need to restart app");
        alert.messageText = NSLocalizedString(@"You must restart BeardieMusic.", @"Title that we need to restart app");
...
- (void)resetStatusMenu{
    ...
    if (!menuItems.count) {
        ...
        if (accessibilityApiEnabled) {
            ...
            item = [statusMenu insertItemWithTitle:NSLocalizedString(@"You must restart BeardieMusic", @"Title on empty menu")
                                                        action:nil
                                                 keyEquivalent:@""
                                                       atIndex:0];
        }
...
//    NSDictionary *options = @{NSValueTransformerNameBindingOption: NSKeyedUnarchiveFromDataTransformerName};
    ...
    [self bind:BeardieMusicPlayPauseShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieMusicPlayPauseShortcut]
       options:options];

    [self bind:BeardieMusicNextTrackShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieMusicNextTrackShortcut]
       options:options];

    [self bind:BeardieMusicPreviousTrackShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieMusicPreviousTrackShortcut]
       options:options];

    [self bind:BeardieMusicActiveTabShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieMusicActiveTabShortcut]
       options:options];

    [self bind:BeardieMusicFavoriteShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieMusicFavoriteShortcut]
       options:options];

    [self bind:BeardieMusicNotificationShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieMusicNotificationShortcut]
       options:options];

    [self bind:BeardieMusicActivatePlayingTabShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieMusicActivatePlayingTabShortcut]
       options:options];
   ...
   withKeyPath:[@"values." stringByAppendingString:BeardieMusicActivatePlayingTabShortcut]
    ...
    [self bind:BeardieMusicPlayerNextShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieMusicPlayerNextShortcut]
       options:options];

    [self bind:BeardieMusicPlayerPreviousShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieMusicPlayerPreviousShortcut]
       options:options];
}
...
- (void)setBeardieMusicPlayPauseShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieMusicPlayPauseShortcut: shortcut}];
    }
}
- (void)setBeardieMusicNextTrackShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieMusicNextTrackShortcut: shortcut}];
    }
}
- (void)setBeardieMusicPreviousTrackShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieMusicPreviousTrackShortcut: shortcut}];
    }
}
- (void)setBeardieMusicActiveTabShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieMusicActiveTabShortcut: shortcut}];
    }
}
- (void)setBeardieMusicFavoriteShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieMusicFavoriteShortcut: shortcut}];
    }
}
- (void)setBeardieMusicNotificationShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieMusicNotificationShortcut: shortcut}];
    }
}
- (void)setBeardieMusicActivatePlayingTabShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieMusicActivatePlayingTabShortcut: shortcut}];
    }
}
- (void)setBeardieMusicPlayerNextShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieMusicPlayerNextShortcut: shortcut}];
    }
}
- (void)setBeardieMusicPlayerPreviousShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieMusicPlayerPreviousShortcut: shortcut}];
    }
}
...
- (BOOL)newConnectionToControlService{
    ...
    if (_connectionToService) {
     ...
     _connectionToService = [[NSXPCConnection alloc] initWithServiceName:@"com.beardiemusic.BeardieMusicControllers"];
     _connectionToService.remoteObjectInterface = [NSXPCInterface interfaceWithProtocol:@protocol(BeardieMusicControllersProtocol)];

    _connectionToService.exportedInterface = [NSXPCInterface interfaceWithProtocol:@protocol(BeardieMusicHostAppProtocol)];
...
- (void)resetShortcutsToControlService{
    ...
    NSData *shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieMusicPlayPauseShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieMusicPlayPauseShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieMusicNextTrackShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieMusicNextTrackShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieMusicPreviousTrackShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieMusicPreviousTrackShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieMusicActiveTabShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieMusicActiveTabShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieMusicFavoriteShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieMusicFavoriteShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieMusicNotificationShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieMusicNotificationShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieMusicActivatePlayingTabShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieMusicActivatePlayingTabShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieMusicPlayerNextShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieMusicPlayerNextShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieMusicPlayerPreviousShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieMusicPlayerPreviousShortcut];
    }
...
- (void)setHeadphonesListener{
    ...
    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setPhoneUnplugActionEnabled:[[NSUserDefaults standardUserDefaults] boolForKey:BeardieMusicRemoveHeadphonesAutopause]];
    }
}
...
- (void)setAppleRemotes{
    ...
    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setUsingAppleRemoteEnabled:[[NSUserDefaults standardUserDefaults] boolForKey:BeardieMusicUsingAppleRemote]];
    }
}
