...
//                        nil];
    ...
    NSDictionary *appDefaults = [NSDictionary dictionaryWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"BeardieUserDefaults" ofType:@"plist"]];
    if (appDefaults)
        [[NSUserDefaults standardUserDefaults] registerDefaults:appDefaults];

    // Create serial queue for user actions
    workingQueue = dispatch_queue_create("com.beardie.working.serial", DISPATCH_QUEUE_SERIAL);
    ...
    [registry setUserDefaults:BeardieActiveControllers strategyCache:strategyCache];

    // setup native apps
    nativeAppRegistry = [NativeAppTabRegistry singleton];
    [nativeAppRegistry setUserDefaultsKey:BeardieActiveNativeAppControllers];
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:BeardieUpdateAtLaunch])
...
/////////////////////////////////////////////////////////////////////////
#pragma mark BeardieHostAppProtocol methods
/////////////////////////////////////////////////////////////////////////
...
- (void)volumeUp{
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:BeardieCustomVolumeControl]) {
...
- (void)checkAXAPIEnabled{
    ...
    if (_AXAPIEnabled){
        ...
        alert.informativeText = NSLocalizedString(@"Once you enable access in System Preferences, you must restart Beardie.", @"Explanation that we need to restart app");
        alert.messageText = NSLocalizedString(@"You must restart Beardie.", @"Title that we need to restart app");
...
- (void)resetStatusMenu{
    ...
    if (!menuItems.count) {
        ...
        if (accessibilityApiEnabled) {
            ...
            item = [statusMenu insertItemWithTitle:NSLocalizedString(@"You must restart Beardie", @"Title on empty menu")
                                                        action:nil
                                                 keyEquivalent:@""
                                                       atIndex:0];
        }
...
//    NSDictionary *options = @{NSValueTransformerNameBindingOption: NSKeyedUnarchiveFromDataTransformerName};
    ...
    [self bind:BeardiePlayPauseShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardiePlayPauseShortcut]
       options:options];

    [self bind:BeardieNextTrackShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieNextTrackShortcut]
       options:options];

    [self bind:BeardiePreviousTrackShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardiePreviousTrackShortcut]
       options:options];

    [self bind:BeardieActiveTabShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieActiveTabShortcut]
       options:options];

    [self bind:BeardieFavoriteShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieFavoriteShortcut]
       options:options];

    [self bind:BeardieNotificationShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieNotificationShortcut]
       options:options];

    [self bind:BeardieActivatePlayingTabShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardieActivatePlayingTabShortcut]
       options:options];
   ...
   withKeyPath:[@"values." stringByAppendingString:BeardieActivatePlayingTabShortcut]
    ...
    [self bind:BeardiePlayerNextShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardiePlayerNextShortcut]
       options:options];

    [self bind:BeardiePlayerPreviousShortcut
      toObject:[NSUserDefaultsController sharedUserDefaultsController]
   withKeyPath:[@"values." stringByAppendingString:BeardiePlayerPreviousShortcut]
       options:options];
}
...
- (void)setBeardiePlayPauseShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardiePlayPauseShortcut: shortcut}];
    }
}
- (void)setBeardieNextTrackShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieNextTrackShortcut: shortcut}];
    }
}
- (void)setBeardiePreviousTrackShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardiePreviousTrackShortcut: shortcut}];
    }
}
- (void)setBeardieActiveTabShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieActiveTabShortcut: shortcut}];
    }
}
- (void)setBeardieFavoriteShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieFavoriteShortcut: shortcut}];
    }
}
- (void)setBeardieNotificationShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieNotificationShortcut: shortcut}];
    }
}
- (void)setBeardieActivatePlayingTabShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardieActivatePlayingTabShortcut: shortcut}];
    }
}
- (void)setBeardiePlayerNextShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardiePlayerNextShortcut: shortcut}];
    }
}
- (void)setBeardiePlayerPreviousShortcut:(NSData *)shortcut{

    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setShortcuts:@{BeardiePlayerPreviousShortcut: shortcut}];
    }
}
...
- (BOOL)newConnectionToControlService{
    ...
    if (_connectionToService) {
     ...
     _connectionToService = [[NSXPCConnection alloc] initWithServiceName:@"com.beardie.BeardieControllers"];
     _connectionToService.remoteObjectInterface = [NSXPCInterface interfaceWithProtocol:@protocol(BeardieControllersProtocol)];

    _connectionToService.exportedInterface = [NSXPCInterface interfaceWithProtocol:@protocol(BeardieHostAppProtocol)];
...
- (void)resetShortcutsToControlService{
    ...
    NSData *shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardiePlayPauseShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardiePlayPauseShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieNextTrackShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieNextTrackShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardiePreviousTrackShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardiePreviousTrackShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieActiveTabShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieActiveTabShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieFavoriteShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieFavoriteShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieNotificationShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieNotificationShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardieActivatePlayingTabShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardieActivatePlayingTabShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardiePlayerNextShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardiePlayerNextShortcut];
    }

    shortcut  = [[NSUserDefaults standardUserDefaults] objectForKey:BeardiePlayerPreviousShortcut];
    if (shortcut) {
        [dict setObject:shortcut forKey:BeardiePlayerPreviousShortcut];
    }
...
- (void)setHeadphonesListener{
    ...
    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setPhoneUnplugActionEnabled:[[NSUserDefaults standardUserDefaults] boolForKey:BeardieRemoveHeadphonesAutopause]];
    }
}
...
- (void)setAppleRemotes{
    ...
    if (_connectionToService) {
        [[_connectionToService remoteObjectProxy] setUsingAppleRemoteEnabled:[[NSUserDefaults standardUserDefaults] boolForKey:BeardieUsingAppleRemote]];
    }
}
